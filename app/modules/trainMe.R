...
trainMe <- function(input, output, session, pool) {
  ...
  observeEvent(input$submit, {
    ...
    ids <- pool %>% tbl("values") %>% select("RATER") %>% as.data.frame()
    if (is.na(input$rater)) {
      showModal(modalDialog(
        title = "Input Error: Invalid",
    ...
    } else if (as.integer(input$rater) %in% ids$RATER) {
      showModal(modalDialog(
        title = "Input Error: Non unique rater id",
        "rater id already exists.",
        easyClose = TRUE, footer = NULL
      ))
    } else {
    ...
    #   ids <- pool %>% tbl("values") %>% select("RATER") %>% as.data.frame()
    #   
    #   if (as.integer(input$rater) %in% ids$RATER) {
    #     showModal(modalDialog(
    #       title = "Input Error: Non unique rater id",
    #       "rater id already exists.",
    #       easyClose = TRUE, footer = NULL
  ...
  observeEvent(input$save, {
    ...
    if (is.na(input$rater)) {
      ...
      showModal(modalDialog(
        title = "Save success!",
        "Training answers recorded to the database.",
        easyClose = TRUE, footer = NULL
      )) 
    }
  })
